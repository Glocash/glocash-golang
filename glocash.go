...
// return a html string, it's displayed a button in page. you shoule insert it in your sale page
func (g *Glocash) PayEmbed(data map[string]string) string {
	p := make(map[string]string)
	urlPath := g.url + EMBED
	for k, _ := range FormEmbed {
		v, ok := data[k]
		if ok {
			p[k] = v
		}
	...
	var script string = "<script src=" + urlPath + " "
	for k, v := range p {
		script += `"` + k + `"="` + v + `" `
	}
	script += " ></script>"
